...
SECCIONES = ["Resumen", "Temporalidad", "Div / Terr", "Detalle Cuenta", "Detalle PDC",
             "PDC & Calor", "Cierres", "Trimestres", "Comparación de Versiones", "Cashi"]
...
if SEC == "Cashi":
    st.markdown("### 🤖 Cashi · Asistente Financiero Inteligente")
    st.markdown("**Pregúntame lo que quieras**: cifras de gasto por CECO, divisiones, nómina, desgloses por semana, acumulados a la semana X, etc.")

    import rag.financial_rag as fin_rag

    q_default = "gasto de division norte desglosado por semana"
    q_user = st.text_input("Ingresa tu consulta en lenguaje natural:", value=q_default, key="cashi_query")
    
    c1, c2, c3, c4 = st.columns(4)
    if c1.button("💼 Nómina CECO 1005", key="btn_cashi_1"):
        q_user = "gasto de nomina para ceco 1005"
    if c2.button("🗺️ División Norte por semana", key="btn_cashi_2"):
        q_user = "gasto de division norte desglosado por semana"
    if c3.button("📅 Acumulado a Sem 29", key="btn_cashi_3"):
        q_user = "gasto acumulado a la semana 29"
    if c4.button("🔗 Match de CECOs", key="btn_cashi_4"):
        q_user = "match de cecos"

    if q_user:
        with st.spinner("Cashi está procesando tu consulta..."):
            engine = fin_rag.FinancialRAGEngine()
            res = engine.query(q_user)
            
            st.subheader(res.get("titulo", "Resultado Financiero"))
            st.markdown(res.get("respuesta", ""))
            
            if "real_2026_mdp" in res:
                k1, k2, k3, k4 = st.columns(4)
                k1.metric("Real 2026", f"{res.get('real_2026_mdp', 0):,.2f} MDP")
                k2.metric("Plan 2026", f"{res.get('plan_2026_mdp', 0):,.2f} MDP")
                k3.metric("Real 2025", f"{res.get('real_2025_mdp', 0):,.2f} MDP")
                v = res.get('vs_plan_mdp', 0)
                k4.metric("Vs Plan", f"{abs(v):,.2f} MDP", delta=f"{v:+.2f} MDP", delta_color="inverse")
            
            tabla_data = res.get("tabla", [])
            if tabla_data:
                df_res = pd.DataFrame(tabla_data)
                for col_m in ["Real_2025", "Plan_2026", "Real_2026"]:
                    if col_m in df_res.columns:
                        df_res[col_m] = df_res[col_m].map(lambda x: f"{x/1e6:,.2f} MDP" if pd.notna(x) else ("—" if pd.isna(x) else x))
